<template>
  <div class="selected-ticket__container" :class="{ 'selected-ticket--activated': markedAsActive, 'already-booked': booked }">
        <div class="selected-ticket__heading">
            ...
            <div class="selected-ticket__controls">
                <button @click.prevent="updateIsTimerActiveState(taskKey)"
                        class="icon-play"
                        :disabled="(isTimerActive && (activeTicket === taskKey))"
                        :class="{'disabled': (isTimerActive && (activeTicket === taskKey)) }">
                </button>
                <button @click.prevent="updateIsTimerActiveState(taskKey)"
                        class="icon-pause"
                        :disabled="(!isTimerActive && (activeTicket !== taskKey)) || (isTimerActive && (activeTicket !== taskKey)) || !isTimerActive"
                        :class="{'disabled': (!isTimerActive && (activeTicket !== taskKey)) || (isTimerActive && (activeTicket !== taskKey)) || !isTimerActive}">
                </button>
                ...
                <button v-b-modal="`confirm-deletion-modal-${taskKey}`" class="icon-trash"></button>
                <b-modal :id="`confirm-deletion-modal-${taskKey}`" centered title="Delete Task?">
                    <div>Delete tracker for {{ taskKey }}: {{ taskSummary }}?</div>
                    <template v-slot:modal-footer="{ ok }">
                      <button @click.prevent="removeTicketFromSelectedTickets(taskKey)" class="icon-trash">Delete</button>
                    </template>
                </b-modal>

            </div>
        </div>
        <button v-b-toggle="`selected-task-${taskKey}`" class="time-collapse"></button>
        <b-collapse :id="`selected-task-${taskKey}`" visible class="selected-ticket__content">
            <textarea rows="4" :value="taskWorklogComment" @input="saveCommentToStore"></textarea>
            ...
            <div class="selected-ticket__tracked-time">
                ...
                <button :class="{ 'icon-pencil': !editingTrackedTime, 'icon-check': editingTrackedTime }" class="navigation__icon" @click.prevent="activateEditModeForTrackedTime"></button>
            </div>
        </b-collapse>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
        ...
        parsedEndTime () {
            if (this.activeTicket === this.taskKey && this.isTimerActive) return 'ongoing...';
    ...
    methods: {
        ...
        bookSingleTaskOnly() {
            this.requestSavingSingleWorklog({ comment: this.taskWorklogComment, timeSpentSeconds: this.timeSpent, ticketId: this.taskKey })
                .then(() => {
        ...
        saveEditedStartTime: _.debounce(function (event) {
            ...
            if (this.parsedEndTime !== '00:00:00') {
                ...
                this.saveTimeSpentOnTask({ taskKey: this.taskKey, timeSpent: updatedTimeDifference });
            }

            this.startTime = editedStartTimeAsDate;
            this.saveTaskStartTime({ taskKey: this.taskKey, startTime: editedStartTimeAsDate.toTimeString() });
        ...
        saveEditedEndTime: _.debounce(function (event) {
            ...
            this.saveTaskEndTime({ taskKey: this.taskKey, endTime: editedNewEndTimeAsDate.toTimeString() });
            this.saveTimeSpentOnTask({ taskKey: this.taskKey, timeSpent: updatedTimeDifference });
            this.saveSelectedTasksToStorage();
        }, 1000),
        saveEditedWorklogTimeSpent: _.debounce(function (event) {
            ...
            if (!(this.parsedStartTime === '00:00:00')) {
                ...
                this.saveTaskEndTime({ taskKey: this.taskKey, endTime: dateFromSummedUpArrayAndHelper.toTimeString() });
            }
            this.saveTimeSpentOnTask({ taskKey: this.taskKey, timeSpent: dateFromSummedUpArrayAndHelper.getTime() - dateFromParsedStartTime.getTime() });
            this.saveSelectedTasksToStorage();
        }, 1000),
        ...
        toggleNameEditing: function (cancelEdit = false) {
            ...
            if (!this.editingName && (this.taskKey !== this.localEditedName) && !_.isEmpty(this.localEditedName) && !cancelEdit) {
                this.assignNameToCustomTask({ assignedTaskKey: this.localEditedName, currentTaskKey: this.taskKey });

                this.saveSelectedTasksToStorage();
            }
        },
        saveEditedCustomTaskName: _.debounce(function (event) {
            if (!_.isEmpty(event.target.value) && (this.localEditedName !== this.taskKey)) this.localEditedName = event.target.value;
        }, 1500),
        toggleTicketAssignment: function (cancelAssignment = false) {
            ...
            if (!this.showSelection && (cancelAssignment === false) && (this.lastSelectedTicket !== this.selectedTicket)) {
                ...
                this.assignToTicket({ taskKey: this.taskKey,  assignedTicketKey: this.selectedTicket });
        ...
        saveCommentToStore: _.debounce(function (event) {
            this.saveTaskComment({ taskKey: this.taskKey, comment: event.target.value });
        ...
        currentTimeInSeconds: function () {
            ...
            this.saveTimeSpentOnTask({ taskKey: this.taskKey, timeSpent: __timeSpent });
        ...
        startTimer: function () {
            ...
            this.saveTaskStartTime({taskKey: this.taskKey, startTime: this.startTime.toTimeString()});
        ...
        stopTimer: function () {
            ...
            this.saveTaskEndTime({taskKey: this.taskKey, endTime: this.endTime.toTimeString()});
      ...
      watch: {
          isTimerActive: function () {
              if ((this.taskKey === this.lastTicket) && this.onABreak) {
                  this.stopTimer();
                  this.setLastTicket('');
                  this.setActiveTicket('');
              }
              if (this.taskKey === this.lastTicket) {
                  this.stopTimer();
                  this.setLastTicket('');
                  this.setIsTimerActive();
              }
